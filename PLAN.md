...
- ターン実行中に成功した逐次的な command execution が続く場合は、カードを追加し続けず、安定した最新コマンド表示へ内容を差し替える。元の command item と item ID は Extension Host 側に保持し、履歴データ自体を上書きまたは削除しない。
...
- 最新カードへの差し替え時は同じ表示枠を再利用し、ユーザーがフォーカスしている要素やスクロール位置を不用意に失わない。全文は表示に必要な範囲だけ Webview へ渡し、コマンド出力や追加の機密情報は公開しない。

完了条件:

- 通常完了したコマンドが連続しても実行中画面へカードが増え続けず、現在または直近のコマンドを1か所で確認できる。
...
- ターン実行中は、正常完了した逐次コマンドを直近の1件へ集約し、並列実行中と失敗・拒否されたコマンドは個別表示を維持するようにした。
...
- 逐次実行、並列実行、失敗、拒否、完了後の履歴保持を単体テストへ追加した。
