...
- thread ID と turn ID の組を `workspaceState` に保存する `TurnBookmarkStore` を追加し、スレッドごとに独立した追加・解除と再読み込み後の復元を実装した。
- 各 turn のヘッダーへキーボード操作可能なブックマークボタンを追加し、状態を `aria-pressed` と明示的な読み上げラベルで通知するようにした。
- 会話上部へ表示中のブックマーク一覧を追加し、選択時に該当 turn へスクロールして turn 自体へフォーカスを移し、移動先をライブリージョンでも案内するようにした。
- turn とブックマーク一覧を ID ベースで差分更新し、ストリーミング通知や履歴再同期で DOM を再利用してキーボードフォーカスを維持するようにした。
- Webview は現在の session／thread／turn に対する固定 toggle メッセージだけを送信でき、Extension Host が存在確認と永続化を所有する境界を維持した。履歴に存在しない保存済み turn は安全に表示対象から除外する。
- Store、Webview プロトコル、Provider の自動テストへ、スレッド分離、永続化、破損状態の無害化、欠落 turn の除外、古い要求の拒否を追加した。
