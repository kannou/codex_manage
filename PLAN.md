...
| 8 | 未着手 | 17.9 新着・完了表示 | スクロール追従と複数スレッドの未確認状態を追加し、長時間利用を改善する。 |
| 9 | 未着手 | 17.8 下書き保持 | thread ごとの本文・添付状態を安全に分離して保持する。 |
| 10 | 未着手 | 17.6 残り使用量パネル | 安定した footer へ App Server の rate limit 取得とパネルを追加する。 |
| 11 | 未着手 | 17.4 `@`／`$` サジェスト | 検索、IME、候補状態、アクセシビリティを含む最も大きい変更として最後に実装する。 |
