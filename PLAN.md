...
- 作成日: 2026-07-14
- 状態: vNext Phase D 手動受け入れ確認保留／Phase F 17.4 実装済み・手動確認待ち
...
- 実行中、完了後、Reload 後の表示モデル、summary 通知の境界検証、別 turn の item ID 衝突を自動テストへ追加した。フォーカスとスクロール位置は手動確認待ち。
...
- Webview からは turn ID と不透明な file ID だけを送り、Extension Host が現在の権威あるターンから対象を再解決してからエディターで開くようにした。作成、更新、削除、移動、重複、複数ルート、外部パス、古い ID、Reload の自動テストを追加した。実際のマウス／キーボード操作と複数ルート表示は手動確認待ち。
...
- App Server 境界、検索相関、ワークスペース外、無効な Skill、候補 ID、添付と turn input、カーソル位置、日本語検索語、重複を自動テストへ追加した。実際の日本語 IME、各テーマ、マウス／キーボード、スクリーンリーダーでの操作は手動確認待ち。
...
- 実行中、完了通知後、Reload 後、失敗、中断、拒否、作業なし、Host-to-Webview 境界を自動テストへ追加した。スクロール位置と実際のキーボード操作は手動確認待ち。
...
- ストリーミング中の追従と位置維持、完了状態、View の可視性とフォーカス、複数スレッド、重複通知、Back、Reload、Host-to-Webview 境界を自動テストへ追加した。実際のサイドバー切り替え、狭い表示、マウス／キーボード操作は手動確認待ち。
...
- Back でキャッシュ済み一覧が更新要求より先に表示されることと、重複要求の集約、追従更新、エラー通知指定の引き継ぎを単体テストへ追加した。新規スレッドの自動タイトル反映と遅い実 App Server での操作感は手動確認待ち。
...
- composer footer には横幅を抑えた `n%` だけを表示する。表示値はコンテキストの残量を表し、ツールチップでは `Context window: n% left`、スクリーンリーダーには同じ意味を省略せず伝える。
...
- 累積 token 数と context window から残量を算出して 0〜100% に制限し、欠落値や不正値では表示しないようにした。圧縮後に残量が増える通知も最新値として反映する。
- composer footer に等幅数字の `n%` を追加し、ツールチップと読み上げ名では `Context window: n% left` と意味を明示した。アカウント利用枠のボタンと表示状態を分離した。
- 通知の境界値と不正値を単体テストへ追加した。実 App Server での resume 後の再通知、狭いサイドバー、コンテキスト圧縮後の表示は手動確認待ち。
...
- 送信中、再開中、開始中、停止中、承認・追加入力待ち、接続エラーは既存の状態文を維持した。自動テストは具体的な3点表示ではなく、実行状態とインジケーター・読み上げ文の対応を検証するようにした。実際のテーマ、狭いサイドバー、動きを抑えるOS設定での表示は手動確認待ち。
...
- 1秒未満、秒表示、1分境界、分単位ちょうど、長時間、欠落値、負数、非有限値の単体テストを追加した。実際のテーマと狭いサイドバーでの表示は手動確認待ち。
...
| 4 | 実装済み・手動確認待ち | 17.12 会話画面からのスレッド名変更 | 既存の名前変更処理を再利用でき、会話中の整理操作を完結できる。 |
| 5 | 実装済み・手動確認待ち | 17.10 コピー操作 | 小さい操作追加で利用頻度が高く、後続の表示再編前に単独で検証しやすい。 |
| 6 | 実装済み・手動確認待ち | 17.7 作業ログ折りたたみ | 完了後の会話構造を整理し、変更ファイル一覧を置く土台を作る。 |
| 7 | 実装済み・手動確認待ち | 17.2 変更ファイル一覧 | 整理後の完了ターンへ集約一覧とファイルオープン操作を追加する。 |
| 8 | 実装済み・手動確認待ち | 17.13 コピーボタンの間隔調整 | 既存のコピー操作を保った小さい CSS 改善として先に反映できる。 |
| 9 | 実装済み・手動確認待ち | 17.14 実行中コマンドの集約 | 長時間ターンの表示量を抑え、作業中の会話を追いやすくする。 |
| 10 | 実装済み・手動確認待ち | 17.9 新着・完了表示 | スクロール追従と複数スレッドの未確認状態を追加し、長時間利用を改善する。 |
| 11 | 実装済み・手動確認待ち | 17.15 モデル変更時の推論レベル維持 | モデル変更時の不要な設定リセットをなくし、次ターンの実行条件を予測しやすくする。 |
| 12 | 実装済み・手動確認待ち | 17.16 実行中作業カードの集約 | 17.14 と 17.9 の状態管理を活かし、長時間ターンで一時的な作業カードが蓄積するのを防ぐ。 |
| 13 | 実装済み・手動確認待ち | 17.8 下書き保持 | thread ごとの本文・添付状態を安全に分離して保持する。 |
| 14 | 実装済み・手動確認待ち | 17.6 残り使用量パネル | 安定した footer へ App Server の rate limit 取得とパネルを追加する。 |
| 15 | 実装済み・手動確認待ち | 17.17 一覧へ戻った際の同期 | 新規スレッドのタイトルなど、現在発生している一覧の不整合を小さい再同期処理で先に解消する。 |
| 16 | 実装済み・手動確認待ち | 17.19 応答中インジケーター | 既存の実行状態と footer を利用した表示改善として、待機中の視認性を先に高める。 |
| 17 | 実装済み・手動確認待ち | 17.20 ターン所要時間 | 既存の turn metadata に限定した単位変換で、完了後の読みやすさを改善する。 |
| 18 | 実装済み・手動確認待ち | 17.18 コンテキスト残量 | footer の配置を確定した後、token usage 通知の検証と thread ごとの状態管理を追加する。 |
| 19 | 実装済み・手動確認待ち | 17.4 `@`／`$` サジェスト | 検索、IME、候補状態、アクセシビリティを含む最も大きい変更として最後に実装する。 |
