...
- turn 単位のブックマークを `workspaceState` に thread ID ごとに保存し、会話の再読み込み後も維持するようにした。
- 各 turn の星形ボタンから登録・解除でき、状態を `aria-pressed` と明示的なラベルで読み上げられるようにした。
- 会話ヘッダーのブックマーク一覧から対象 turn へスクロールしてキーボードフォーカスを移し、移動結果をライブリージョンで通知するようにした。
- Webview からの要求を現在の session、thread、実在する turn に相関付け、任意の保存状態を変更できない境界を維持した。
- Store と Webview protocol に、スレッド間分離、永続化、不正状態の無害化、境界検証の単体テストを追加した。
