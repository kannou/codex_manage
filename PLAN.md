...
- ターン実行中の command execution は実行中だけカードを表示し、正常完了した時点で Codex の応答完了を待たずに非表示にする。元の command item と item ID は Extension Host 側に保持し、履歴データ自体を上書きまたは削除しない。
...
- 通常完了したコマンドは Codex の応答が続いていても実行中画面から消え、現在実行中のコマンドだけを確認できる。
...
- ターン実行中は、正常完了したコマンドを完了通知の反映時点で非表示にし、並列実行中と失敗・拒否されたコマンドは個別表示を維持するようにした。
...
- 逐次実行、コマンド完了後も応答が継続する場合、並列実行、失敗、拒否、完了後の履歴保持を単体テストへ追加した。
