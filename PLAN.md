...
- 作成日: 2026-07-14
- 状態: vNext Phase D 手動受け入れ確認保留／Phase F 完了
...
- thread ID と turn ID の組を `workspaceState` に保存し、スレッドごとの追加・解除と再読み込み後の復元を実装した。
- 各 turn のブックマークボタンへ状態ラベルを付け、固定ヘッダーのプレビュー付き一覧から対象 turn へフォーカス移動できるようにした。
- 欠落 turn の除外、現在の session／thread／turn の検証、重複更新の抑止、保存失敗の処理を Extension Host 側へ追加した。
- 会話ヘッダーの Back と Reload をアクセシブルなアイコンボタンへ変更した。View title の Refresh Threads は一覧全体を更新する別機能として維持した。
...
- [x] turn をブックマークし、再読み込み後も対象箇所へ移動できる。
- [x] Phase E の追加後も既存の一覧管理、会話、承認、再同期、セキュリティ境界が回帰しない。

## 17. vNext Phase F: 会話表示の仕上げ

### 17.1 Reasoning summary の進行状態に応じた表示

実装方針:

- ターンの実行中は `Reasoning summary` という見出しだけを表示せず、受信した summary 本文を展開した状態で直接表示する。
- ターンの完了後、権威ある完成データに reasoning item が残っている場合は、summary 本文を折りたたみ可能な表示へ切り替える。
- 完成データに reasoning item が含まれない場合は、実行中に表示していた provisional な summary をそのまま消し、拡張側で独自に保存・復元しない。
- 実行中から完了後の表示へ切り替える際も、同じ turn／item ID に相関付け、別スレッドや別ターンの summary を混在させない。

完了条件:

- summary のストリーミング中は、追加操作なしで最新の本文を読める。
- 完了データに summary があれば折りたたんで残り、なければカード全体が消える。
- 完了通知が部分データの場合は、既存の自動収束後に得た権威ある履歴を基準として最終表示を決める。
- 実行中、完了後、Reload 後の3状態を自動テストし、フォーカスとスクロール位置に不要な変化がないことを手動確認する。

実装結果（2026-07-22）:

- `item/reasoning/summaryPartAdded` と `item/reasoning/summaryTextDelta` を検証して取り込み、thread／turn／item ID の所有関係を維持したまま summary をストリーミング表示するようにした。
- 実行中の summary は見出しを隠した展開表示とし、完了後は同じ item の DOM を再利用して閉じた折りたたみへ切り替えるようにした。空の summary カードは表示しない。
- 部分的な完了通知では provisional summary を既存の自動収束まで維持し、権威ある full history に reasoning item がなければ削除することを自動テストへ追加した。
- 実行中、完了後、Reload 後の表示モデル、summary 通知の境界検証、別 turn の item ID 衝突を自動テストへ追加した。フォーカスとスクロール位置は手動確認済み。

### 17.2 ターン完了後の変更ファイル一覧

実装方針:

- ターンの実行中に届く個別の file change カードは途中経過として維持し、ターン完了後に同じターンの変更ファイルを重複除去した一覧カードを結論の後へ追加する。
- 一覧はワークスペースからの相対パスで表示し、作成、更新、削除、移動の種別を識別できるようにする。移動後のファイルは最終パスを開く対象とし、削除済みファイルは削除状態を表示してリンクにしない。
- 相対パスと絶対パスを Extension Host 側で正規化し、現在開いているワークスペースフォルダー内に解決できる変更だけを一覧へ含める。`/tmp` などワークスペース外のファイルは表示も操作も行わない。
- ファイル選択時に Webview から任意のパスを受け取らない。Webview は turn ID と検証済み項目の ID だけを送り、Extension Host が現在の一覧に対応する URI を再検証して VS Code エディターで開く。
- 完了通知が部分データの場合は provisional な変更一覧を確定扱いせず、既存の自動収束で得た権威ある完成データから一覧を構築する。

完了条件:

- 1ターン内で複数の file change item に同じファイルが現れても、完了後の一覧には1件だけ表示される。
- 一覧の既存ファイルをマウスまたはキーボードで選択すると、対応するワークスペース内ファイルが VS Code エディターで開く。
- ワークスペース外、パス走査、別ターンの古い項目 ID は表示またはオープンできない。
- 作成、更新、削除、移動、重複、ワークスペース外、Reload 後の一覧を自動テストし、複数ワークスペースフォルダーでも正しい相対パスになることを確認する。

実装結果（2026-07-24）:

- 完了済みかつ full history のターンだけを対象に、複数の file change item から変更先ファイルを重複除去し、最終回答の後へ `Changed files (件数)` の一覧を表示するようにした。
- 作成、更新、削除、移動を区別し、移動は最終パスを対象とした。削除済みファイルは状態だけを表示して選択不可とし、一覧項目はマウスとキーボードで操作できるボタンにした。
- Extension Host で相対パスと絶対パスを正規化し、ワークスペース内の変更だけを表示するようにした。複数ルートではフォルダー名を含む相対パスとし、パス走査とワークスペース外の変更を除外した。
- Webview からは turn ID と不透明な file ID だけを送り、Extension Host が現在の権威あるターンから対象を再解決してからエディターで開くようにした。作成、更新、削除、移動、重複、複数ルート、外部パス、古い ID、Reload の自動テストを追加した。実際のマウス／キーボード操作と複数ルート表示は手動確認済み。
- 一覧カードは初期状態を閉じた折りたたみとし、変更種別は横幅を抑えた Git 形式の `A`、`M`、`D`、`R` で表示するようにした。記号にはテーマ色を補助的に使い、ツールチップと読み上げでは正式な種別名を維持した。

### 17.3 プロンプト入力欄へ移動するキーボードショートカット

実装方針:

- VS Code コマンド `Codex Thread Manager: Focus Conversation Prompt` を追加し、会話画面で入力可能な composer がある場合にプロンプト入力欄へフォーカスを移す。
- 会話画面が開かれたままサイドバーが非表示の場合は、対象 View を表示してから入力欄へフォーカスする。一覧画面、読み込み中、切断中など入力できない状態ではフォーカスを移さない。
- デフォルトのキーバインドは Windows／Linux で `Ctrl+Alt+Enter`、macOS で `Cmd+Alt+Enter` とし、会話画面が開いていることを示す context key で有効範囲を限定する。ユーザーは VS Code の Keyboard Shortcuts から上書きまたは解除できる。
- Extension Host は固定されたフォーカスメッセージだけを Webview へ送り、Webview 側は現在表示中の composer と入力可能状態を再確認してから既存の textarea をフォーカスする。

完了条件:

- エディター、会話履歴、サイドバー外へフォーカスがある状態から、ショートカットで現在の会話の入力欄へ移動できる。
- サイドバーが非表示でも同じショートカットで会話画面を表示し、入力欄へ移動できる。
...
- manifest、コマンド登録、context key の切り替え、Host-to-Webview 境界を自動テストへ追加した。各OSの実キー操作、サイドバー非表示からの復帰、ユーザーによる再割り当ては手動確認済み。

### 17.4 `@` ファイル・`$` Skill サジェスト

実装方針:

- プロンプト入力中、カーソル直前の単語境界にある `@検索語` でワークスペース内ファイル、`$検索語` で現在の workspace `cwd` に対して有効な Skill の候補を入力欄の上へ表示する。
- ファイル候補は App Server の fuzzy file search を現在開いているワークスペースフォルダーに限定して使用し、Skill 候補は既存の `skills/list` を再利用する。候補数と検索頻度に上限を設け、新しい検索を開始したら古い結果を画面へ適用しない。
- 候補を選択したら、入力中の `@検索語` または `$検索語` だけを textarea から取り除き、既存のファイル／Skill 添付チップへ変換する。textarea 自体は plain text のままとし、文中へリッチなチップを埋め込む editor への置き換えは行わない。
- Add メニューと既存のファイル／Skill picker は残し、マウス操作や候補検索を使わない場合の導線として同じ添付状態を更新する。
- 候補は Arrow Up／Down、Enter または Tab、Escape、マウスで操作でき、選択中候補と件数をスクリーンリーダーへ通知する。IME composition 中は検索・選択を開始せず、変換確定後に再評価する。
- Webview には候補 ID、表示名、ワークスペース相対パス、Skill の説明だけを渡す。選択時は候補 ID を Extension Host 側の現在の検索結果と照合し、任意のパスや Skill を Webview から指定できないようにする。

完了条件:

- 文頭と文中の `@` からワークスペース内ファイルを検索・選択でき、既存のファイル添付と同じ内容で次の turn に送信される。
- `$` から有効な Skill を名前と説明で検索・選択でき、既存の Skill 添付と同じ `UserInput.skill` として送信される。
- 候補を閉じた場合や一致する候補がない場合は入力文字を勝手に削除せず、通常のテキストとして編集できる。
- ワークスペース外のファイル、無効な Skill、古い検索結果、別スレッドの候補 ID を選択または送信できない。
- カーソル移動、複数の候補指定、重複、送信失敗後の保持、日本語 IME、キーボード操作、Add メニューとの併用を自動テストと手動チェックで確認する。

実装結果（2026-07-25）:

- composer のカーソル直前にある単語境界の `@検索語` と `$検索語` を検出し、150ms の待機後に最大20件の候補を入力欄の上へ表示するようにした。カーソル移動、選択範囲、入力変更のたびに再評価し、日本語 IME の composition 中は検索も候補選択も行わない。
- ファイル候補は App Server の `fuzzyFileSearch` を現在開いているワークスペースフォルダーだけに限定して呼び出し、ディレクトリ、パス走査、ワークスペース外、重複結果を除外した。Skill 候補は現在の `cwd` に対する `skills/list` の有効項目を名前と説明で絞り込む。
- Webview へは不透明な候補 ID と表示用の相対パス／Skill 説明だけを渡し、選択時に Extension Host が最新検索、session、thread、候補 ID、ファイル状態を再検証して既存の添付状態へ追加するようにした。古い検索結果や別スレッドの候補は選択できない。
- Arrow Up／Down、Enter、Tab、Escape、マウス選択、listbox の選択状態と件数読み上げを追加した。選択成功時だけ該当する `@検索語`／`$検索語` を plain text の textarea から除き、一致なし、Escape、失敗時は入力を保持する。Add、Runtime settings、使用量パネルとの排他表示も維持した。
- App Server 境界、検索相関、ワークスペース外、無効な Skill、候補 ID、添付と turn input、カーソル位置、日本語検索語、重複を自動テストへ追加した。実際の日本語 IME、各テーマ、マウス／キーボード、スクリーンリーダーでの操作は手動確認済み。

### 17.5 composer footer の配置

実装方針:

- composer footer を左側の補助情報領域と右側の送信操作領域に分け、状態文の有無にかかわらず Send／Stop を常に右端へ配置する。
- 左側には残り使用量ボタンを置く領域を確保し、Codex の応答待ちや入力要求を示す状態文はその右隣に表示する。状態文が空でも左側領域と右端の送信操作の位置を変えない。

完了条件:

- idle、応答待ち、停止中、入力要求、利用不能の各状態で Send／Stop が同じ右端位置を維持する。
- 状態文が左側のアイコン領域の直後に表示され、狭いサイドバーでも送信操作を押し出さず、長い状態文は安全に折り返す。

実装結果（2026-07-22）:

- composer footer に左側の情報領域を追加し、状態文をその内側へ移動した。残り使用量ボタンは Phase F 順位9で同じ領域へ追加できる構造とした。
- Send／Stop の操作領域へ `margin-left: auto` を設定し、状態文が空の idle 時を含めて常に右端へ配置した。
- idle、応答待ち、停止中、入力要求、利用不能、狭いサイドバーでの配置確認を手動チェックリストへ追加した。

### 17.6 残り使用量パネル

実装方針:

- composer footer の左端へ残り使用量を開くアイコンボタンを置く。文字ラベルを画面へ出さず、既存のアイコンボタンと同じテーマ色、ツールチップ、アクセシブルな名前、フォーカス表示を持たせる。
- ボタンを選択すると composer の上へコンパクトなパネルを開き、`account/rateLimits/read` が返す primary／secondary window の残量、リセット時刻、利用可能な場合は credits と個人使用上限を表示する。残量は `usedPercent` から算出し、期間名が不明な場合は推測で「5時間」「週間」などの名前を付けない。
- 使用量はパネルを開いたときに取得して最新値を保持し、`account/rateLimits/updated` 通知が届いた場合は公開された値だけをマージする。ターンごとのポーリングは行わない。
- 未ログイン、API key 利用、App Server の未対応、取得失敗などで値を得られない場合は `Usage unavailable` と再試行可能な状態を表示し、0%または100%として扱わない。
- 使用量パネルはボタン再選択、外側クリック、Escape で閉じ、Escape では使用量ボタンへフォーカスを戻す。Add／Runtime settings／サジェストとは同時に開かない。

完了条件:

- 使用量パネルに取得可能な各 window の残量とローカル時刻のリセット予定が表示され、更新通知または再表示で最新値へ更新される。
- 値の欠落、範囲外の割合、不正なリセット時刻、取得失敗を境界検証し、誤解を招く残量を表示しない。
- アイコンのマウス、キーボード、スクリーンリーダー操作と、外側クリック／Escape／他メニューとの排他表示を自動テストと手動チェックで確認する。

実装結果（2026-07-24）:

- composer footer 左端のアイコンから残り使用量パネルを開き、primary／secondary window、credits、個人使用上限の公開値を表示するようにした。
- パネルを開くたびに `account/rateLimits/read` で取得し、`account/rateLimits/updated` の検証済み通知を表示へ反映するようにした。
- 欠落値、範囲外の割合、不正なリセット時刻、取得失敗を残量として表示せず、再試行可能な `Usage unavailable` 状態にした。
- 外側クリック、Escape、Add／Runtime settings との排他表示、フォーカス復元を追加した。

### 17.7 ターン完了後の作業ログ折りたたみ

実装方針:

- ターンの実行中は command、file change、MCP、plan などの作業カードを個別に表示し、進捗をリアルタイムで確認できる状態を維持する。
- ターン完了後は同じターンの完了済み作業カードを `Work details (件数)` の1つの折りたたみへまとめ、最終回答と変更ファイル一覧を常に外側へ表示する。
- 失敗または中断された作業がある場合は折りたたみ見出しへ状態を表示し、詳細を開かなくても問題があったことを識別できるようにする。
- ライブ表示から完了表示へ切り替える際は既存の turn／item ID を維持し、ユーザーが開いた折りたたみ、フォーカス、スクロール位置を不用意に失わない。

完了条件:

- 実行中は各作業の進捗が見え、完了後は作業ログが1か所へまとまって最終回答を追いやすくなる。
- 作業がないターンには空の折りたたみを表示せず、失敗または中断があるターンでは見出しだけで状態を確認できる。
- 完了通知、権威ある履歴への自動収束、Reload の後も同じ作業だけが正しいターンへ表示される。

実装結果（2026-07-23）:

- 実行中は既存の作業カードを個別表示し、完了後は同じ activity item の DOM を `Work details (件数)` の折りたたみへ移動して、メッセージと最終回答を外側へ残すようにした。
- 作業の失敗、中断、拒否を折りたたみ見出しの状態バッジへ表示し、作業がないターンには折りたたみを表示しないようにした。
- ライブ表示から完了表示へ切り替える際に item ID、内側カードの開閉状態、キーボードフォーカスを維持し、以後の差分更新でもユーザーが開いた `Work details` を閉じないようにした。
- 実行中、完了通知後、Reload 後、失敗、中断、拒否、作業なし、Host-to-Webview 境界を自動テストへ追加した。スクロール位置と実際のキーボード操作は手動確認済み。

### 17.8 スレッドごとのプロンプト下書き保持

実装方針:

- 未送信のプロンプト本文と選択済み添付を thread ID ごとに Extension Host で保持し、Back または別スレッドへ移動した後に同じスレッドへ戻ると復元する。
- 初期スコープは同じ VS Code ウィンドウを開いている間の保持とし、ファイルパスや Skill パスを Webview state や永続ストレージへ保存しない。ウィンドウ再読み込みをまたぐ永続化は別途必要性を確認してから検討する。
- turn の開始が受理された場合だけ対象スレッドの下書きを消去し、送信失敗、切断、別画面への遷移では保持する。
- 削除・アーカイブされたスレッド、存在しなくなった添付、上限や capability が変わって無効になった添付は安全に除外し、理由を表示する。

完了条件:

- 本文と画像、ファイル、Skill を選んだ状態で Back または別スレッドへ移動しても、戻ったときに同じ下書きが復元される。
- 送信成功時だけ下書きが消え、失敗時は再送できる。別スレッドの本文や添付は混在しない。
- Webview state、ログ、永続ストレージへローカルパスを保存しないことを境界テストで確認する。

実装結果（2026-07-25）:

- Extension Host に thread ID 単位のインメモリ下書きを追加し、本文と画像、ファイル、Skill を Back やスレッド切り替え後に復元するようにした。Webview state は従来どおりナビゲーション情報だけを保持する。
- turn 開始の受理時だけ下書きを削除し、送信失敗、切断、画面遷移では保持するようにした。新規スレッド作成後に最初の turn だけ失敗した場合も、確定した thread ID へ下書きを引き継ぐ。
- 復元時に存在しないファイル、種別ごとの上限超過、重複、選択モデルで利用できない画像を除外して理由を表示し、削除・アーカイブされたスレッドの下書きを破棄するようにした。
- スレッド間の分離、本文と3種の添付の復元、失敗時の保持、再同期後の送信成功時の消去、無効添付とアーカイブ時の破棄、Webview 境界でパスを公開・永続化しないことを自動テストへ追加した。

### 17.9 新着・完了表示と最新箇所への移動

実装方針:

- ユーザーが会話下端から離れている間に新しい本文や作業項目が届いた場合、現在位置を強制的に動かさず、会話下部へ移動する `Latest` ボタンを表示する。
- サイドバーが非表示または別画面へフォーカスしている間にターンが完了した場合は、Codex View に控えめな未確認バッジを表示し、トースト通知は既定で出さない。
- `Latest` の選択、対象会話の最終ターン確認、別スレッドへの移動、Reload に応じて、現在の thread／turn に対応する表示だけを解除する。

完了条件:

- 過去の発言を読んでいる最中は自動スクロールせず、新着があることと最新箇所へ移動する手段を確認できる。
- 非表示中に完了したスレッドをバッジから確認でき、確認済みまたは別スレッドの完了を未読として残さない。
- ストリーミング、複数スレッド、Back、Reload、サイドバー表示切り替えで未読状態が混在しない。

実装結果（2026-07-25）:

- 会話下端付近では従来どおり新着へ追従し、下端から離れている場合はスクロール位置を維持して composer 上へ `Latest` ボタンを表示するようにした。ボタン選択または手動で下端へ移動すると、最新ターンへ移動して表示を解除する。
- ターン完了を Extension Host で thread ID と turn ID に相関付けて保持し、Webview View の非表示中または別画面へフォーカス中に完了したスレッド数を Codex View のバッジへ表示するようにした。一覧では対象スレッドだけに `New` を表示する。
- 対象会話の完了ターンを下端で確認した場合、`Latest` を選択した場合、または Reload した場合だけ該当スレッドの未確認状態を解除する。別スレッドの状態、重複した完了通知、不正な session／thread／turn ID は混在させず、トースト通知と永続化は追加していない。
- 現在の一覧または表示中の会話に属さない完了通知はバッジへ加えず、一覧更新で対象外になった未確認状態も破棄するようにした。0件へ戻す際は VS Code の Webview View activity が残る挙動を避けるため、非表示の0件状態を経由して Activity Bar の表示を確実に消去する。
- ストリーミング中の追従と位置維持、完了状態、View の可視性とフォーカス、複数スレッド、重複通知、Back、Reload、Host-to-Webview 境界を自動テストへ追加した。実際のサイドバー切り替え、狭い表示、マウス／キーボード操作は手動確認済み。

### 17.10 最終回答とコードのコピー操作

実装方針:

- Codex の最終回答と Markdown の fenced code block に、マウスとキーボードで利用できるコピーボタンを表示する。
- コピー対象は表示用 HTML ではなく元のプレーンテキストまたはコード文字列とし、固定された Webview メッセージを Extension Host 側で検証して VS Code のクリップボードへ書き込む。
- コピー成功または失敗をボタンのラベルとスクリーンリーダー向けステータスで短く通知し、会話内容を Output Channel へ記録しない。

完了条件:

- 最終回答全体と各コードブロックを個別にコピーでき、改行、インデント、Unicode が保持される。
- ユーザー発言、作業カード、非表示の raw reasoning へコピー操作を追加せず、任意の VS Code コマンドを Webview から実行できない。
- コピー成功・失敗時もスクロール位置とキーボードフォーカスを失わない。

実装結果（2026-07-23）:

- 完了ターンの Codex 最終回答と各 fenced code block に、キーボード操作可能なコピーボタンを追加した。
- Webview は相関済みの session／thread／turn／item ID とコードブロック番号だけを送り、Extension Host が保持する原文を検証して VS Code のクリップボードへ書き込むようにした。
- 成功・失敗をボタンとスクリーンリーダー向けステータスへ短時間表示し、フォーカスとスクロール位置を維持するようにした。
- 改行、インデント、Unicode、未閉鎖 fence、および不正なコピー要求の境界テストを追加した。
- コピーボタンは重なった用紙を表す小さな SVG アイコンとして回答またはコード枠の左下端へ配置し、マウスオーバー、キーボードフォーカス、コピー結果の表示中だけ前面へ出るように調整した。

### 17.12 会話画面からのスレッド名変更

実装方針:

- 会話画面ヘッダーのスレッド名をクリックすると、現在名を初期値にした編集欄へ切り替える。
- 編集欄はマウスだけでなく、キーボードの Enter／Space からも開始でき、Enter で確定、Escape でキャンセルする。
- 確定時は既存の名前変更コマンド／Thread Repository を経由して `thread/name/set` を呼び出し、Webview から任意の App Server メソッドを指定できない構造を維持する。
- 一覧のスナップショット、会話ヘッダー、復元対象のスレッド名を成功後に同じ値へ更新する。サーバー通知や外部変更で更新された場合も、編集中でなければ表示へ反映する。
- 名前は既存の名前変更と同じく前後の空白を除去し、空文字を拒否する。失敗時は編集前の表示を維持し、再試行可能なエラーを表示する。
- 編集欄と確定／キャンセル状態にアクセシブルな名前、フォーカス表示、スクリーンリーダー向けの状態通知を付ける。

完了条件:

- 会話画面のスレッド名をクリックまたはキーボード操作で編集し、保存できる。
- 保存した名前が会話ヘッダー、一覧、再読み込み後の表示で一致する。
- 空文字、通信失敗、切断、編集中の外部更新を安全に扱い、別スレッドへ名前変更が混入しない。
- 確定、キャンセル、失敗、通知更新、フォーカス復元を自動テストし、マウスとキーボードの操作を手動確認する。

実装結果（2026-07-22）:

- 会話ヘッダーのスレッド名をマウスまたはキーボードで編集でき、Enter で保存、Escape でキャンセルできるようにした。
- Webview からは相関済みの thread／session ID と検証済みの名前だけを受け取り、既存の Thread Repository の名前変更処理へ接続した。
- 空文字、古いセッション、保存失敗を拒否し、成功時は会話、一覧、セッションのタイトルを同期してフォーカスを編集ボタンへ戻すようにした。
- プロトコル境界と Extension Host の名前変更フローを自動テストへ追加した。

### 17.13 コピーボタンと回答本文の間隔調整

実装方針:

- Codex の最終回答へ表示するコピーボタンを現在の左下配置のまま少し下へ移し、回答枠の下線がアイコン中央を横切らず、本文とアイコンの間に十分な余白ができるようにする。
- ボタンを下へ移した分だけ回答カード下側の余白も確保し、次の作業カード、変更ファイル一覧、ターン境界と重ならないようにする。コードブロック用コピーボタンは読みやすさを確認し、必要がなければ現在位置を維持する。
- マウスオーバー、キーボードフォーカス、コピー結果表示、タッチ環境での可視性は既存の挙動を維持し、位置調整によってフォーカスリングが切れないようにする。

完了条件:

- 回答本文とコピーボタンの間隔が広がり、回答枠の下線がアイコン中央ではなく上側を通るか、視覚的に分離して見える。
- 狭いサイドバー、長い回答、回答直後に別カードが続く場合でも、ボタンが本文や次の要素へ重ならない。
- マウス、キーボード、タッチ環境でコピー操作と成功・失敗表示を利用できる。

実装結果（2026-07-23）:

- 最終回答のコピーボタンを回答枠の下線より下へ移し、下線がアイコン中央を横切らない配置に調整した。
- 回答カードの下側に余白を追加し、後続の作業カード、変更ファイル一覧、ターン境界とコピーボタンが重ならないようにした。
- コードブロック用ボタンの位置と、既存の hover、キーボードフォーカス、コピー結果、タッチ環境での表示挙動は維持した。

### 17.14 実行中コマンドの集約と長いコマンドの省略表示

実装方針:

- ターン実行中の command execution は正常完了後も一時的にカードを残し、次のコマンドが始まるか、次の Codex の発言が表示された時点で非表示にする。元の command item と item ID は Extension Host 側に保持し、履歴データ自体を上書きまたは削除しない。
- 同時に実行中のコマンドは個別に表示し、失敗、拒否、中断など確認が必要なカードは新しいコマンドが始まっても隠さない。通常完了した過去コマンドだけを実行中表示から省略する。
- ターン完了後は Phase F 17.7 の `Work details` に全 command item を含め、実行中に省略されたコマンドも後から確認できるようにする。Reload 後も権威ある履歴から同じ完了表示を再構築する。
- コマンド文字列が表示幅で2行を超える場合は末尾を省略し、カードを展開すると全文表示へ切り替える。文字数で途中切断せず CSS の行数制限を使い、狭いサイドバーと長い引数に追従させる。
- 完了カードを非表示にする際は、ユーザーのスクロール位置を不用意に失わない。全文は表示に必要な範囲だけ Webview へ渡し、コマンド出力や追加の機密情報は公開しない。

完了条件:

- 通常完了したコマンドは次のコマンドまたは Codex の発言が始まるまで確認でき、その後は実行中画面から消える。
- 並列実行中のコマンドと失敗、拒否、中断したコマンドは省略されず、ターン完了後の `Work details` には全履歴が残る。
- 長いコマンドは閉じた状態で2行以内に収まり、マウスまたはキーボードで展開すると全文を確認できる。
- 逐次実行、並列実行、失敗、拒否、中断、完了通知、Reload、長い Unicode 引数を自動テストし、差し替え時のフォーカスとスクロール位置を手動確認する。

実装結果（2026-07-24）:

- ターン実行中は、正常完了したコマンドの表示状態を通知順序に基づいて Extension Host 側で保持し、空の応答開始通知では消さず、次のコマンド開始または実際の Codex 発言が届いた時点で非表示にするようにした。並列実行中と失敗・拒否されたコマンドは個別表示を維持する。
- 権威ある items 履歴は View Model に保持したまま表示対象 ID だけを分離し、ターン完了時と Reload 後の `Work details` では全コマンドを確認できるようにした。
- コマンド名を閉じた状態で CSS により2行へ省略し、カードを展開すると Unicode を含む全文を表示できるようにした。
- 逐次実行、コマンド完了後の待機、空の応答開始、次の Codex 発言、並列実行、失敗、拒否、完了後の履歴保持を単体テストへ追加した。

### 17.15 モデル変更時の推論レベル維持

実装方針:

- スレッド内でモデルを変更したとき、変更先モデルが現在選択中の推論レベルをサポートしていれば、その値を維持して次のターンへ適用する。モデル変更を理由に推論レベルを `null` へ戻さない。
- 変更先モデルが現在の推論レベルをサポートしていない場合だけ、App Server のモデル情報が示すデフォルト値へフォールバックし、実際に選択された値を UI へ即座に反映する。サポート外の値を送信したり、表示と送信値が異なる状態にはしない。
- モデル一覧と各モデルの対応推論レベルを権威ある情報として扱い、モデル設定の通知、会話画面の再描画、スレッドの再開後も同じ選択状態を表示する。
- 推論レベル以外の runtime 設定は、この変更に伴って意図せず変更しない。

完了条件:

- 共通の推論レベルをサポートするモデル間では、モデル変更後も選択中の推論レベルが維持される。
- 変更先モデルが現在値をサポートしない場合は、そのモデルのデフォルト値へ安全に切り替わり、UI と次ターンの送信値が一致する。
- モデルを連続して変更した場合、設定通知を受けた場合、スレッドを再読み込みした場合でも、推論レベルが意図せずデフォルトへ戻らない。
- 対応値を維持する場合、非対応値へフォールバックする場合、モデル情報が欠ける場合を自動テストする。

実装結果（2026-07-25）:

- スレッド内のモデル変更を Extension Host 側で検出し、現在の明示的な推論レベルが変更先モデルでもサポートされている場合は、その値を維持するようにした。
- 変更先モデルが現在の推論レベルをサポートしていない場合は `Default` へ戻し、変更先モデルが示すデフォルト値を UI と次ターンへ適用するようにした。
- モデル ID と対応推論レベルは App Server の検証済みモデルカタログから解決し、同じモデル内での推論レベル変更と、権限や speed など他の runtime 設定の更新は既存の挙動を維持した。
- 対応モデル間での維持と、非対応モデルへのフォールバックを単体テストへ追加した。

### 17.16 実行中作業カードの集約

17.14 のコマンド集約を、ターン実行中に表示する他の作業カードへ拡張する。

実装方針:

- 正常完了した一時的な作業カードは、次の意味のある作業カードが始まるか、次の実際の Codex 発言が表示された時点で実行中画面から非表示にする。たとえば、コマンド実行開始、コマンド完了、ファイル変更開始の順に通知された場合は、ファイル変更開始時に直前のコマンドカードを非表示にする。
- 最初の対象は command execution、file change、web search、MCP tool call、dynamic tool call、image view／generation、wait とする。Web 調査が MCP または dynamic tool として通知される場合も同じ規則で扱う。
- plan、reasoning summary、review mode、context compaction、hook、collaboration／sub-agent、未知の将来 item は、会話の文脈、状態遷移、注意事項を表す可能性があるため、一律には非表示にしない。必要性を確認してから種類ごとに対象へ追加する。
- 並列に実行中のカードはすべて表示し、失敗、拒否、中断、承認待ちなどユーザーの確認が必要なカードは、後続の作業カードが始まっても残す。正常完了した一時カードだけを入れ替え対象にする。
- 権威ある item と item ID は Extension Host 側へ保持し、表示対象だけを切り替える。ターン完了後の `Work details` には省略したカードを含む全作業履歴を表示し、Reload 後も復元できるようにする。
- カードの入れ替えによって、ユーザーのスクロール位置、展開状態、キーボードフォーカスを不用意に失わない。

完了条件:

- command から file change、file change から web search／tool call、tool call から Codex 発言へ進む場合に、正常完了した直前の一時カードが実行中画面から消える。
- 同時実行中の作業、失敗、拒否、中断、承認待ちのカードは非表示にならず、後続の状態を確認できる。
- plan、reasoning summary、状態遷移を示すカードは、一時的な実行カードの入れ替えに巻き込まれない。
- ターン完了後の `Work details` と Reload 後の表示では、実行中に省略した全作業履歴を確認できる。
- 種類をまたぐ逐次実行、並列実行、エラー、Codex 発言への切り替え、Reload、フォーカスとスクロール位置を自動テストまたは手動確認する。

実装結果（2026-07-25）:

- 17.14 のコマンド専用表示状態を、command execution、file change、web search、MCP／dynamic tool call、image view／generation、wait を対象とする一時作業カードの表示状態へ拡張した。
- 正常完了した一時作業カードは次の可視な作業カード、reasoning summary、または実際の Codex 発言が始まるまで表示し、その後は実行中画面から省略するようにした。空の Codex 応答開始や空の reasoning 開始だけでは省略しない。
- 通知 Reducer の item lifecycle と item ID を使って省略対象を管理し、同時に実行中のカードと、失敗、拒否など確認が必要なカードは後続の作業が始まっても表示を維持するようにした。
- 元の item 履歴は削除せず、ターン完了後の `Work details` と変更ファイル一覧では全履歴を再構築できる既存構造を維持した。
- command、file change、web search、MCP failure、reasoning／Codex 発言への切り替え、並列実行中カードの保持を単体テストへ追加した。

### 17.17 会話から一覧へ戻った際のバックグラウンド同期

実装方針:

- 会話画面から Back でスレッド一覧へ戻る際は、保持中の一覧を即座に表示したうえで、既存の `thread/list` 更新処理をバックグラウンドで実行する。通信完了まで画面遷移を待たせたり、一覧を空にしたりしない。
- `thread/started` や `thread/name/updated` などの通知による通常の更新を維持し、Back 時の取得は通知漏れや遅延によって残った古いタイトル、状態、並び順を収束させるための再同期として扱う。
- Back の連続操作や手動 Reload と取得が重なった場合は、同じ更新を無制限に並行実行せず、古い応答で新しい一覧を上書きしない。取得中に別スレッドを開いた場合も、完了した一覧更新によって会話画面を一覧へ戻さない。
- 更新に失敗した場合は保持中の一覧をそのまま利用可能にし、Back 操作ごとにトーストを表示しない。既存の接続状態と手動 Reload から再試行できるようにする。
- グループの展開状態、ピン留め、未確認表示は一覧データの再取得と分離し、バックグラウンド同期によって不用意に初期化しない。

完了条件:

- 新規スレッドの作成後に一覧へ戻ると、生成済みのタイトル、最新状態、並び順が手動 Reload なしで反映される。
- Back を選択するとキャッシュ済み一覧が即座に表示され、遅い応答や取得失敗でも一覧操作を継続できる。
- Back、手動 Reload、通知更新、別スレッドへの遷移が競合しても、古い一覧への巻き戻りや意図しない画面遷移が起きない。
- 成功、失敗、連続 Back、遅延した応答、タイトル通知との前後関係を自動テストする。

実装結果（2026-07-25）:

- 会話画面の Back では保持中の一覧を先に表示し、その直後に Extension Host へバックグラウンド更新を要求するようにした。初期表示や不正な復元先から一覧へ戻る経路では不要な更新を開始しない。
- 起動時、手動 Reload、設定・ワークスペース変更、Back からの一覧更新を共通の直列実行キューへ接続した。実行中に重なった要求は次の1回へ集約し、手動 Reload のエラー通知要求も失わないようにした。
- 既存の generation と client 相関を維持し、古い取得結果を表示へ反映しないようにした。Back 時の取得失敗はトーストを出さず、先に表示した一覧と既存の再試行操作を維持する。
- Back でキャッシュ済み一覧が更新要求より先に表示されることと、重複要求の集約、追従更新、エラー通知指定の引き継ぎを単体テストへ追加した。新規スレッドの自動タイトル反映と遅い実 App Server での操作感は手動確認済み。

### 17.18 コンテキストウィンドウ残量の表示

実装方針:

- App Server の `thread/tokenUsage/updated` を境界検証し、thread ID と turn ID に相関付けた最新の token usage と `modelContextWindow` から、現在のスレッドで利用可能なコンテキストの割合を算出する。
- composer footer には横幅を抑えた `n%` だけを表示する。表示値はコンテキストの残量を表し、ツールチップとスクリーンリーダーでは `Context window: n% left (残り token / 利用可能 token)` と算出根拠も伝える。
- `n%` は桁数の変化で footer が大きく揺れない等幅数字とし、既存のアカウント利用枠とは別の値であることをツールチップと詳細表示で明確にする。
- token usage または context window が欠ける場合、不正な場合、0以下の場合は割合を推測せず非表示にする。割合は0〜100%へ安全に制限し、コンテキスト圧縮後に残量が増える更新も最新値として反映する。
- 使用量はスレッドごとに分離し、別スレッドの通知や古い turn の値を現在の footer へ混在させない。スレッドの resume 時に再通知された保存済み使用量から表示を復元する。

完了条件:

- token usage 通知を受けると、現在のスレッドの footer に `n%` 形式でコンテキスト残量が表示される。
- 表示だけでは省略した意味をツールチップとスクリーンリーダーで確認でき、アカウントの primary／secondary window と混同しない。
- スレッド切り替え、resume、ターン更新、コンテキスト圧縮、欠落値、不正値、別スレッド通知を安全に扱う。
- 狭いサイドバーと1桁、2桁、3桁の割合で Send／Stop や状態表示を押し出さない。

実装結果（2026-07-25）:

- `thread/tokenUsage/updated` の thread ID、turn ID、token usage、context window を境界検証し、現在のスレッドの最新ターンに一致する通知だけを保持するようにした。
- 最新リクエストの token 数と context window から固定領域を除いて残量を算出し、0〜100% に制限した。欠落値や不正値では表示せず、圧縮後に現在コンテキストが減った通知も最新値として反映する。
- composer footer に等幅数字の `n%` を追加し、ツールチップと読み上げ名には残り token 数と固定領域を除いた利用可能 token 数も表示した。アカウント利用枠のボタンと表示状態を分離した。
- 通知の境界値と不正値を単体テストへ追加した。実 App Server での resume 後の再通知、狭いサイドバー、コンテキスト圧縮後の表示は手動確認済み。

### 17.19 応答中のアクティビティインジケーター

実装方針:

- Codex の通常応答中は composer footer の `Codex is responding…` という可視テキストを表示せず、同じ位置へコンパクトなアクティビティインジケーターを表示する。
- 初期表示は横に並んだ3点が左から順に明るくなるアニメーションとし、上下移動によるレイアウトの揺れを避ける。幅は footer の状態領域へ収まる最小限にする。
- DOM と状態管理は見た目に依存しない汎用的な `activity-indicator` とし、TypeScript は実行状態に応じた表示切り替えだけを担当する。点の形、動き、速度は CSS の疑似要素と animation へ分離し、後からパルス、スピナー、波形などへ差し替える際にプロトコルや状態管理を変更しない。
- アニメーション自体は読み上げ対象外とし、非表示テキストと既存の live region で `Codex is responding` を通知する。`prefers-reduced-motion` では動かない状態表示へ切り替える。
- 停止中、承認・追加入力待ち、接続エラーなど、ユーザーの操作や注意が必要な状態はアニメーションだけに置き換えず、既存の状態文を維持する。

完了条件:

- turn の実行中は footer にアクティビティインジケーターが表示され、完了、失敗、中断後は消える。
- 通常応答中の可視テキストを省略しても、スクリーンリーダーから状態を確認できる。
- 動きを抑える設定、狭いサイドバー、Send／Stop の切り替えでレイアウトが揺れない。
- 自動テストは実行状態と表示・読み上げ状態の対応を検証し、3点という具体的な見た目へ依存させない。

実装結果（2026-07-25）:

- 通常の `running` 状態では composer footer の可視テキストを非表示にし、同じ位置へ横並びの3点が順に明るくなるアクティビティインジケーターを表示するようにした。完了、失敗、中断後は実行状態に従って表示を解除する。
- DOM は見た目に依存しない `conversation-activity-indicator` と3個の子要素で構成し、各点の動きを CSS の keyframes へ分離した。テーマ色の差に依存せず、3点が時間差で明暗を変えることで動きを判別できるようにした。後から表示方式を変更しても状態管理やプロトコルを変更しない構造にした。
- インジケーターを読み上げ対象外とし、既存の live region には非表示の `Codex is responding` を維持した。VS Code の `workbench.reduceMotion` が `on` の場合は3点を静止表示し、`auto` の場合だけ `prefers-reduced-motion` に従い、`off` の場合はWebview内でもアニメーションを維持する。
- 送信中、再開中、開始中、停止中、承認・追加入力待ち、接続エラーは既存の状態文を維持した。自動テストは具体的な3点表示ではなく、実行状態とインジケーター・読み上げ文の対応を検証するようにした。実際のテーマ、狭いサイドバー、動きを抑えるOS設定での表示は手動確認済み。

### 17.20 ターン所要時間の人間向け表示

実装方針:

- Codex の応答完了までにかかった turn 全体の `durationMs` は、ミリ秒の生値ではなく、人間が読み取りやすい秒・分表示へ変換する。
- 1秒未満は `<1 sec`、1分未満は整数の `n sec`、1分以上は `n min` または `n min n sec` とし、`0.04 min` のような小数の分表示を避ける。
- 初期対象はターンのメタデータに表示する応答時間とし、command、MCP、wait など個別作業項目の実行時間はこの変更へ混在させない。
- `null`、負数、非有限値は所要時間として表示せず、非常に長い時間でも桁あふれや不自然な単位表示を起こさない。

完了条件:

- 短い応答は秒、1分以上の応答は分と秒で表示され、ターンメタデータに `ms` が表示されない。
- 1秒未満、1分境界、分単位ちょうど、長時間、欠落値、不正値を単体テストする。
- 日時、履歴状態など同じメタデータ行の他の値と区切りやレイアウトが崩れない。

実装結果（2026-07-25）:

- turn metadata の `durationMs` を専用 formatter に通し、1秒未満を `<1 sec`、1分未満を整数秒、1分以上を整数の分と端数秒で表示するようにした。日時と履歴状態は既存の中黒区切りを維持する。
- formatter は `null`、負数、非有限値を表示対象外とし、秒未満の端数を切り捨てることで単位境界を越える丸めを避けた。個別の command、MCP、wait の duration 表示には影響させていない。
- 1秒未満、秒表示、1分境界、分単位ちょうど、長時間、欠落値、負数、非有限値の単体テストを追加した。実際のテーマと狭いサイドバーでの表示は手動確認済み。

### 17.11 Phase F の対応順

Phase F は、小さく独立した改善から表示構造と状態管理を固め、App Server 連携と入力サジェストを後段で実装する。

| 順位 | 状態 | 対象 | 理由 |
| --- | --- | --- | --- |
| 1 | 完了 | 17.5 composer footer の配置 | CSS と DOM の小さな変更で Send／Stop の位置を先に安定させる。 |
| 2 | 完了 | 17.1 Reasoning summary | 既存の reasoning item と turn 状態を使う表示改善で、日常の読みやすさへの効果が大きい。 |
| 3 | 完了 | 17.3 入力欄フォーカスのショートカット | 既存 textarea のフォーカス処理を再利用でき、他の composer 改善から独立している。 |
| 4 | 完了 | 17.12 会話画面からのスレッド名変更 | 既存の名前変更処理を再利用でき、会話中の整理操作を完結できる。 |
| 5 | 完了 | 17.10 コピー操作 | 小さい操作追加で利用頻度が高く、後続の表示再編前に単独で検証しやすい。 |
| 6 | 完了 | 17.7 作業ログ折りたたみ | 完了後の会話構造を整理し、変更ファイル一覧を置く土台を作る。 |
| 7 | 完了 | 17.2 変更ファイル一覧 | 整理後の完了ターンへ集約一覧とファイルオープン操作を追加する。 |
| 8 | 完了 | 17.13 コピーボタンの間隔調整 | 既存のコピー操作を保った小さい CSS 改善として先に反映できる。 |
| 9 | 完了 | 17.14 実行中コマンドの集約 | 長時間ターンの表示量を抑え、作業中の会話を追いやすくする。 |
| 10 | 完了 | 17.9 新着・完了表示 | スクロール追従と複数スレッドの未確認状態を追加し、長時間利用を改善する。 |
| 11 | 完了 | 17.15 モデル変更時の推論レベル維持 | モデル変更時の不要な設定リセットをなくし、次ターンの実行条件を予測しやすくする。 |
| 12 | 完了 | 17.16 実行中作業カードの集約 | 17.14 と 17.9 の状態管理を活かし、長時間ターンで一時的な作業カードが蓄積するのを防ぐ。 |
| 13 | 完了 | 17.8 下書き保持 | thread ごとの本文・添付状態を安全に分離して保持する。 |
| 14 | 完了 | 17.6 残り使用量パネル | 安定した footer へ App Server の rate limit 取得とパネルを追加する。 |
| 15 | 完了 | 17.17 一覧へ戻った際の同期 | 新規スレッドのタイトルなど、現在発生している一覧の不整合を小さい再同期処理で先に解消する。 |
| 16 | 完了 | 17.19 応答中インジケーター | 既存の実行状態と footer を利用した表示改善として、待機中の視認性を先に高める。 |
| 17 | 完了 | 17.20 ターン所要時間 | 既存の turn metadata に限定した単位変換で、完了後の読みやすさを改善する。 |
| 18 | 完了 | 17.18 コンテキスト残量 | footer の配置を確定した後、token usage 通知の検証と thread ごとの状態管理を追加する。 |
| 19 | 完了 | 17.4 `@`／`$` サジェスト | 検索、IME、候補状態、アクセシビリティを含む最も大きい変更として最後に実装する。 |
